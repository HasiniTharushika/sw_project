...
const SchemaFormItem = (props: SchemaFormItemProps) => {
  ...
  const fType = useMemo(() => {
    const fType2 = field.type;
    if (typeof fType2 === 'string') {
      return fType2;
    }

    if (fType2.hasOwnProperty('object')) {
      return 'object';
    }
    return 'array';
  }, [field.type]);
  ...
  const options = useMemo(() => {
    ...
    const optionsx = {...field.options};
    if (optionsx.dependencies) {
      optionsx.dependencies = optionsx.dependencies.map((d: any) =>
        typeof d === 'string' ? [...props.parentPath, d] : d,
      );
    }
    optionsx.rules = optionsx.rules.map((rule: any) => {
      if (
      ...
      return (formi: FormInstance) => ({
        ...rule,
        message,
        validator: async (rulei: any, value: any) =>
          adapter.validator(rulei, value, formi, props.parentPath, field),
      });
    });
    return optionsx;
...
> = props => {
  ...
  const lastSetFieldHandler = useRef<any>(undefined);
  ...
  useEffect(() => {
    if (props.form) {
      if (lastSetFieldHandler.current !== props.form.setFields) {
        const setFields = props.form.setFields;
        props.form.setFields = (fields: any) => {
          setFields(fields);
          setErrors(fields, true);
        };
        lastSetFieldHandler.current = props.form.setFields;
      }
    }
  ...
  //   useCallback(
  //     params => {
  //       if (params.action === 'ERROR' && params.data.form === props.form) {
  //         setErrors(params.data.errors);
  //       }
  //     },
  //     [props.form, setErrors],
  //   ),
  ...
  if (error.name === 'AxiosError') {
    if (error?.response?.data) {
      const err = Object.entries(
        (error?.response.data as any).validationErrors,
      ).map(e => ({
        name: e[0].split('.'),
